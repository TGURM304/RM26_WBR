...
% ==================== 几何参数 ====================
R_val = 0.058;              % 轮子半径 (m)
R_w_val = 0.4386/2;  % 轮距/2 (m)

% ==================== 机体参数 ====================
m_b_val = 16.5;            % 机体质量 (kg)
I_b_val = 403455/1000/1000;            % 机体俯仰转动惯量 (kg·m²)
l_b_val = 12.3/1000;            % 机体质心到俯仰轴距离 (m)
I_yaw_val = 543186.7/1000/1000;          % 整体yaw轴转动惯量 (kg·m²)
theta_b0 = -0.1079;               % 质心偏移角度，单位：弧度

% ==================== 轮子参数 (分别定义左右) ====================
m_wl_val = 0.627;            % 左轮质量 (kg)
m_wr_val = 0.627;            % 右轮质量 (kg)
I_wl_val = 656.352/1000/1000;     % 左轮转动惯量 (kg·m²)
I_wr_val = 656.352/1000/1000;     % 右轮转动惯量 (kg·m²)

% ==================== 腿部参数 (分别定义左右, 默认腿长 0.20m) ====================

% l_l_val = 0.171143;             % 左腿长度 (m)
% l_r_val = 0.171143;             % 右腿长度 (m)
% m_l_val = 0.785;             % 左腿质量 (kg)
% m_r_val = 0.785;             % 右腿质量 (kg)
% I_l_val = 0.021595072;           % 左腿转动惯量 (kg·m²)
% I_r_val = 0.021595072;           % 右腿转动惯量 (kg·m²)
% l_l_d_val = 0.12645;  % 左腿质心到轮轴距离 (m)
% l_r_d_val = 0.12645;  % 右腿质心到轮轴距离 (m)
% theta_l0 = 1.182295785;  % 左腿偏移角度，单位：弧度
% theta_r0 = 1.182295785;  % 右腿偏移角度，单位：弧度

l_l_val = 0.218108;             % 左腿长度 (m)
l_r_val = 0.218108;             % 右腿长度 (m)
m_l_val = 0.785;             % 左腿质量 (kg)
m_r_val = 0.785;             % 右腿质量 (kg)
I_l_val = 0.02412508;           % 左腿转动惯量 (kg·m²)
I_r_val = 0.02412508;           % 右腿转动惯量 (kg·m²)
l_l_d_val = 0.1453;  % 左腿质心到轮轴距离 (m)
l_r_d_val = 0.1453;  % 右腿质心到轮轴距离 (m)
theta_l0 = 0.964003435;  % 左腿偏移角度，单位：弧度
theta_r0 = 0.964003435;  % 右腿偏移角度，单位：弧度

% l_l_val = 0.274466;             % 左腿长度 (m)
% l_r_val = 0.274466;             % 右腿长度 (m)
% m_l_val = 0.785;             % 左腿质量 (kg)
% m_r_val = 0.785;             % 右腿质量 (kg)
% I_l_val = 0.027359656;           % 左腿转动惯量 (kg·m²)
% I_r_val = 0.027359656;           % 右腿转动惯量 (kg·m²)
% l_l_d_val = 0.17088;  % 左腿质心到轮轴距离 (m)
% l_r_d_val = 0.17088;  % 右腿质心到轮轴距离 (m)
% theta_l0 = 0.749303014;  % 左腿偏移角度，单位：弧度
% theta_r0 = 0.749303014;  % 右腿偏移角度，单位：弧度

% l_l_val = 0.321431;             % 左腿长度 (m)
% l_r_val = 0.321431;             % 右腿长度 (m)
% m_l_val = 0.785;             % 左腿质量 (kg)
% m_r_val = 0.785;             % 右腿质量 (kg)
% I_l_val = 0.030635065;           % 左腿转动惯量 (kg·m²)
% I_r_val = 0.030635065;           % 右腿转动惯量 (kg·m²)
% l_l_d_val = 0.19349;  % 左腿质心到轮轴距离 (m)
% l_r_d_val = 0.19349;  % 右腿质心到轮轴距离 (m)
% theta_l0 = 0.593268929;  % 左腿偏移角度，单位：弧度
% theta_r0 = 0.593268929;  % 右腿偏移角度，单位：弧度

% l_l_val = 0.340217;             % 左腿长度 (m)
% l_r_val = 0.340217;             % 右腿长度 (m)
% m_l_val = 0.785;             % 左腿质量 (kg)
% m_r_val = 0.785;             % 右腿质量 (kg)
% I_l_val = 0.032093821;           % 左腿转动惯量 (kg·m²)
% I_r_val = 0.032093821;           % 右腿转动惯量 (kg·m²)
% l_l_d_val = 0.20274;  % 左腿质心到轮轴距离 (m)
% l_r_d_val = 0.20274;  % 右腿质心到轮轴距离 (m)
% theta_l0 = 0.533933616;  % 左腿偏移角度，单位：弧度
% theta_r0 = 0.533933616;  % 右腿偏移角度，单位：弧度
fprintf('  ✓ 物理参数设置完成\n');
...
% lqr_Q = diag([80,    15,      1000,    1,      300,     30,       300,     30,       2000,    30]);
% 柔软LQR起立下的参数
lqr_Q = diag([10,    3,      300,    1,      20,     3,       20,     3,       40,    4]);
...
Leg_data = [
0.16175,    0.12314,    0.03861,      0.021205072;
0.171143,   0.12645,    0.04470,    0.021595072;
0.180536,   0.12994,    0.05059,    0.022005072;
0.189929,   0.13360,    0.05633,    0.022425072;
0.199322,   0.13738,    0.06194,     0.022875072;
0.208715,   0.14129,    0.06742,    0.02365948;
0.218108,   0.14530,    0.07281,    0.02412508;
0.227501,   0.14939,    0.07811,    0.024611649;
0.236894,   0.15357,    0.08333,    0.025119204;
0.246287,   0.15781,    0.08848,    0.025647763;
0.25568,    0.16212,    0.09356,    0.026197343;
0.265073,   0.16647,    0.09860,    0.026767967;
0.274466,   0.17088,    0.10358,    0.027359656;
0.283859,   0.17533,    0.10853,    0.027972436;
0.293252,   0.17982,    0.11343,    0.028606335;
0.302645,   0.18435,    0.11830,    0.029261382;
0.312038,   0.18891,    0.12313,    0.029937613;
0.321431,   0.19349,    0.12794,    0.030635065;
0.330824,   0.19810,    0.13272,     0.031353785;
0.340217,   0.20274,    0.13748,    0.032093821;

];

enable_fitting = false;  % 设为 false 跳过腿长拟合
    ...
    for i = 1:num_legs
        for j = 1:num_legs
            ...
            try
                ...
                K_sample_2d(idx, 3:42) = temp(:)';  % 按行展开
            catch
