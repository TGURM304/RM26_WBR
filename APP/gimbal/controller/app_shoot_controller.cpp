...
void Gimbal::Shoot::shoot_tick() {
    ...
    if(shoot_state_.fric_flag == E_FRIC_REST) {
        ...
        fric_left_out_ =
            fric_left_speed_.update(motor_left_->status.speed, fric_target_speed_);
        fric_right_out_ =
            fric_right_speed_.update(motor_right_->status.speed, -fric_target_speed_);
    }
    if(shoot_state_.trigger_flag == E_TRIGGER_REST)
        ...
        trigger_out_ =
            trigger_out_filter_.process(
                trigger_speed_.update(motor_trigger_->status.speed, trigger_target_speed_));\
